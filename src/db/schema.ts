...
export const toolCategories = sqliteTable(
	...
	(table) => [
		index("tool_categories_name_idx").on(table.name),
		index("tool_categories_sort_idx").on(table.sortOrder),
	],
);

// ─── 🆕 工具条目 ────────────────────────────────────────────────────────────

export const toolItems = sqliteTable(
	"tool_items",
	{
		id: integer("id").primaryKey({ autoIncrement: true }),
	...
	(table) => [
		index("tool_items_category_idx").on(table.categoryId),
		index("tool_items_name_idx").on(table.name),
		index("tool_items_sort_idx").on(table.sortOrder),
	],
);
...
export const notes = sqliteTable(
  "notes",
  {
    id: integer("id").primaryKey({ autoIncrement: true }),
    content: text("content").notNull(),
    // 随机位置：left 百分比, top 百分比
    positionLeft: integer("position_left").notNull().default(10),
    positionTop: integer("position_top").notNull().default(10),
    // 随机旋转角度 (-15deg ~ 15deg)
    rotation: integer("rotation").notNull().default(0),
    // 随机颜色主题
    colorTheme: text("color_theme").notNull().default("yellow"),
    // 是否置顶（被点击后置顶）
